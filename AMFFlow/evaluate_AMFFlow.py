...
@torch.no_grad()
def validate_sintel(model, cfg, step=0):
    ...
    for dstype in ['final', "clean"]:
        ...
        for item in tqdm(val_loader):
            ...
            for ti in range(images.shape[1]-frames+1):
                ...
                flow_pre = padder.unpad(flow_pre[0,-1,-1]).cpu()
                
                epe = torch.sum((flow_pre - flows[0, ti+frames-2]) ** 2, dim=0).sqrt()
        ...
        with open('./logs/results_sintel.txt','a',encoding = 'utf-8') as file:
            w_content = textwrap.fill("Step:(%d)Validation (%s) EPE: %f, 1px: %f, 3px: %f, 5px: %f" % (step,dstype, epe, px1, px3, px5),width = 100)
            file.write("\n")
...
@torch.no_grad()
def validate_spring(model, cfg, split='train'):
...
@torch.no_grad()
def create_spring_submission(model, cfg, output_path='output'):
...
def validate_kitti(model, cfg):
    ...
    val_dataset = datasets.KITTI(aug_params=None,input_frames = cfg.input_frames)
    val_loader = data.DataLoader(val_dataset, batch_size=1, pin_memory=True, shuffle=False, num_workers=2)
    ...
    for item in tqdm(val_loader):
        ...
        flow_pre = processor.step(images)
            ...
            for ti in range(images.shape[1]-frames+1):
                ...
                if ti ==0 :
                    for i in range(frames-1):
                        flow_pre_0 = padder.unpad(flow_pre[0,-1,i]).cpu()
                        _flow = flow_pre_0.permute(1, 2, 0).numpy()
                        ...
                        image.save(f'vis_sintel/{output_path}/{dstype}/flow/{sequence}/{i}.png')
                        imageio.imwrite(f'vis_sintel/gt/{dstype}/image/{sequence}/{i}.png', ((images[0, i].cpu().permute(1, 2, 0).numpy() + 1) * 127.5).astype(np.uint8))

                        output_dir = os.path.join('sintel_submission', output_path, dstype, sequence)
                        output_file = os.path.join(output_dir, 'frame%04d.flo' % (i))
                    ...
                    image.save(f'vis_sintel/{output_path}/{dstype}/flow/{sequence}/{ti+frames-2}.png')
                    imageio.imwrite(f'vis_sintel/gt/{dstype}/image/{sequence}/{ti+frames-2}.png', ((images[0, ti+frames-2].cpu().permute(1, 2, 0).numpy() + 1) * 127.5).astype(np.uint8))

                    output_dir = os.path.join('sintel_submission', output_path, dstype, sequence)
                    output_file = os.path.join(output_dir, 'frame%04d.flo' % (ti+frames-2))
...
@torch.no_grad()
def create_kitti_submission(model, cfg, output_path='output'):
    model.eval()
    results = {}
