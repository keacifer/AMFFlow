...
import os
os.environ["CUDA_VISIBLE_DEVICES"] = ','.join(map(str,[7,6,5,4,3,2,1]))
import random
...
class SpringTrain(FlowDatasetTest):
    def __init__(self, return_gt=True, subsample_groundtruth=True, split='train'):
        ...
        root = '/data4/datasets/spring/train'
...
class ThingsTEST(FlowDatasetTest):
    def __init__(self, return_gt=True, dstype='frames_cleanpass'):
        ...
        root = '/data4/datasets/FlyingThings3D/'
...
class MpiSintel_submission(FlowDatasetTest):
    def __init__(self, return_gt=False, root='/data4/datasets/Sintel', dstype='clean'):
...
class FlyingThings3D(FlowDataset):
    def __init__(self, aug_params=None, input_frames=5, root='/data4/datasets/FlyingThings3D/flow_data/', dstype='frames_cleanpass', forward_warp=False):
    ...
    def __init__(self, aug_params=None, input_frames=3, forward_warp=False):
        ...
        for idx_list in range(200):
            self.image_list.append(
                    [(root + "KITTI-full/training/image_2/000{:03}_{:02}.png".format(idx_list, 2  + 10-input_frames+i)) for
                     i in range(input_frames)])
            self.flow_list.append(
                    [root + "KITTI/training/flow_occ/000{:03}_10.png".format(idx_list)] )
            self.has_gt_list.append(
                    [[False] * (input_frames-2) + [True] ])
...
class KITTISubmission(FlowDatasetTest):
    def __init__(self, return_gt=False):
        ...
        root = '/data4/datasets/KITTI/KITTI-multiview/testing/'
...
def fetch_dataloader(args, TRAIN_DS='C+T+K+S+H', DDP=False):
    ...
    train_loader = data.DataLoader(train_dataset, batch_size=args.batch_size,
                                       pin_memory=True, shuffle=False, num_workers=8, drop_last=True)
