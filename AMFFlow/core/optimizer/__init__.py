...
def build_optimizer(model, config):
    ...
    if name == "adam":
        ...
        elif config.split_train:
            print('use split_train !!!')

            param_dicts = [
                {"params": [p for n, p in model.named_parameters() if "AMF" not in n and p.requires_grad], "lr": config.pretrained_lr ,},
                
                {"params": [p for n, p in model.named_parameters()if "AMF"  in n  and p.requires_grad],   "lr": config.new_lr ,},
                    
            ]
            return torch.optim.AdamW(param_dicts, weight_decay=config.adamw_decay, eps=config.epsilon)
        else:
    ...
    if name == 'OneCycleLR':
        if hasattr(config, 'twins_lr_factor'):
            ...
            scheduler = OneCycleLR(optimizer, [lr, lr*factor], config.num_steps+100,
                pct_start=config.pct_start, cycle_momentum=False, anneal_strategy=config.anneal_strategy)
        else:
            if config.pct_start:
                ...
                print('no pct_start')
                scheduler = OneCycleLR(optimizer, lr, config.num_steps+100,
                    cycle_momentum=False, anneal_strategy=config.anneal_strategy)
    else:
